...
il double Get_diff(int r, int g, int b, int dr, int dg, int db)
{
	return ((r - dr) * 0.28) * ((r - dr) * 0.28) +
		   ((g - dg) * 0.35) * ((g - dg) * 0.35) +
		   ((b - db) * 0.37) * ((b - db) * 0.37);
}
...
int main()
{
	...
	for (ri i = 1; i <= m; i++)
	{
		for (ri j = 1; j <= n; j++)
		{
			int r = read(), g = read(), b = read(), now = inf;
			double QwQ = inf;
			for (ri k = 0; k < 32; k++)
			{
				int dr = dic[k].r, dg = dic[k].g, db = dic[k].b;
				double tmp = Get_diff(r, g, b, dr, dg, db);
				if (tmp < QwQ)
				{
					QwQ = tmp;
					now = k;
