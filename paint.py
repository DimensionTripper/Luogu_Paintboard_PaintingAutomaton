...
myheader = {
    'referer' : 'https://www.luogu.org/paintBoard/board',
    'User-agent' : 'orz_ViXbob'
}
    ...
    if stat == False:
        ...
        try:
            ...
            r = requests.post("https://www.luogu.org/paintBoard/paint",
                              data = data, cookies = user.cookie_dict,
                              headers = myheader)
        if str(r.status_code).find("200") == -1:
            out = Fore.RED
        ...
        print(out + "ret_code:{0}".format(r.status_code))
        print(out + "ret_json:{0}".format(r.json));
        with open("temp.log", "w") as temp:
            temp.write("ret_text:{0}".format(r.text))
